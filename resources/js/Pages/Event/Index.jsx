...
export default function Index({ auth, categories, title, headerOptions, apiToken, eventsFromServer }) {
  ...
  const handleModalBtnClick = (eventId, isClosing = false) => {
    ...
    if (isClosing) {
      setModalState({
        ...modalState, isOpen: true, eventId: +eventId
      });
    } else {
      setModalState({
        ...modalState, isOpen: false, eventId: undefined
      });
    }
  };
  ...
  const addEvent = async (event, data) => {
    ...
    const newEvent = {
      ...data, id: events.length > 0 ? events.length + 1 : 1
    };
  ...
  return (
    <AuthenticatedLayout
      ...
      actionBar={<ActionBar headerOptions={headerOptions} handleChange={handleModalBtnClick} />}
    >
      ...
      <div className="square-pattern-bg ">
        ...
        {modalState['eventId'] && <EventCategoryModalComponent
        ...
        <List
          ...
          sx={{ width: '100%', padding: '15px', display: "flex", justifyContent: "center", alignItems: "center" }}
        >
          <Box
            sx={{ width: '60%', padding: '15px', display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}
          >
            <ExistingCategories events={events} onClick={handleModalBtnClick} />
            <EnterNewCategory isSubmitting={isSubmitting} addEvent={addEvent} />
          </Box>
