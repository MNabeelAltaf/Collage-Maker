...
const ActionBar = ({ handleChange, actions, headerOptions, isCreateButtonDisabled, children }) => {
  return (
    <Stack direction={'row'}>
        ...
        }
        {
          ...
          Object.entries(headerOptions).map(([key, value]) => {
            ...
            if (value['type'] == "action") {
              return (
                <TextButtonLink key={key} isCreateButtonDisabled={isCreateButtonDisabled} id={'header-btn-' + value['title']} type={value['type']} action={actions[value['title']]}>
                  {value['title']}
                </TextButtonLink>
              )
            }
