...
class TemplateController extends Controller
{
    ...
    function readAll(Request $request)
    {
        ...
        foreach (Category::with(['templates' => function ($query) {
            $query
                ->where('created_at', '<', '2024-09-03')
                ->take(5);
        }
        ])->get() as $category) {
            if (
                count($category['templates']) > 0
            ) {
        ...
        $orderLookup = array_flip($categoriesBasedOnEvents);
        usort($response, function ($a, $b) use ($orderLookup) {
            $aOrder = $orderLookup[$a['name']] ?? PHP_INT_MAX;
            $bOrder = $orderLookup[$b['name']] ?? PHP_INT_MAX;
    ...
    function updateImage(Request $request, $uni)
    {
        ...
        if ($request->hasFile('image') && $request->purpose === 'updateImage') {
            ...
            if ($request->type === 'image') {
                ...
                $previouseImage = asset(Storage::url($target->image_2));
                $previouseName = pathinfo($target->image_2, PATHINFO_FILENAME);
            ...
            $newFileName = $previouseName . random_int(1111, 9999) . "." . $newExtension;
            if (Storage::exists($previouseImage) && $request->type !== 'image') {
